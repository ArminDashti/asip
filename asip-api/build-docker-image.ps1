<#
.SYNOPSIS
    Build the as-ip API Docker image.

.DESCRIPTION
    Builds the asip-api:latest image from docker-compose.yml in the project root.
    Optionally exports the image to .deploy/asip-api.tar for remote deployment.
...
function Show-BuildDockerImageHelp {
    Write-Host @'
as-ip Docker build - build the API container image

Usage:
  .\build-docker-image.ps1 [--export=<no|yes>] [--no-cache=<no|yes>] [--help]

Arguments:
  --export=<no|yes>     Export image to .deploy/asip-api.tar after build (default: no)
...
Image:  asip-api:latest
Output: .deploy/asip-api.tar (when --export=yes)
...
function Write-BuildStep {
    ...
    Write-Progress -Activity 'as-ip Docker build' -Status $Message -PercentComplete $percent
...
function Invoke-BuildDockerImage {
    ...
    Write-Progress -Activity 'as-ip Docker build' -Completed -Status 'Done'
    Write-Host ''
...
try {
    ...
    Write-Progress -Activity 'as-ip Docker build' -Completed -Status 'Failed'
    Write-Host ''
